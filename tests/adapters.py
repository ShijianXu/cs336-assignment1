...
from cs336_basics.modules import Linear, Embedding, RMSNorm, SiLU, SwiGLU, RotaryPositionalEmbedding, Softmax
...
def run_embedding(
    ...
    m.load_state_dict({"embed_matrix": weights})
    return m(token_ids)
...
def run_swiglu(
    ...
    m = SwiGLU(d_model=d_model, d_ff=d_ff)
    m.load_state_dict({
        "w1": w1_weight,
        "w2": w2_weight,
        "w3": w3_weight,
